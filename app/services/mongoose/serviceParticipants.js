...
const { otpMail } = require("../email/serviceMail")
...
const activateParticipant = async (req) => {
  ...
  if (!check) throw new NotFoundError('Partisipan not Registered');

  if (check && check.otp !== otp) throw new BadRequestError('Please Check your OTP Again');
...
const signinParticipant = async (req) => {
  ...
  if (result.status === 'tidak aktif') {
    throw new UnAuthorizedError('This Account Have not Acctive');
  }
...
const checkoutOrder = async (req) => {
  ...
  if (!checkingEvent) {
    throw new NotFoundError('There is no Event with ID : ' + event);
  }
